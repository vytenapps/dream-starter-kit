...
function extractChatId(pathname: string): string | null {
  ...
  return pathname.slice(CHAT_PATH.length + 1).split("/")[0] || null;
}
...
export function ActiveChatProvider({ children }: { children: ReactNode }) {
  ...
  // KIT ADAPTATION: model choice persists in localStorage (upstream used a
  // cookie set by a server action).
  useEffect(() => {
    const stored = window.localStorage.getItem("chat-model");
    if (stored) {
      setCurrentModelId(stored);
    }
  }, []);

  useEffect(() => {
    currentModelIdRef.current = currentModelId;
    window.localStorage.setItem("chat-model", currentModelId);
  }, [currentModelId]);
