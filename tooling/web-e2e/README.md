...
| `paywall.spec.ts`       | Anonymous-first identity + the premium-content paywall: favoriting as a logged-out visitor mints a real anonymous account (`public.content_favorites`); a premium post is gated (blurred body + on-page dock); a free user gaining an active subscription unlocks the body (the `usePremium` gating flip); and — when a test-mode `STRIPE_SECRET_KEY` is set — the billing `express-intent` route (the embedded paywall's backend) creates a real Stripe subscription intent. The full embedded Apple/Google-Pay + `4242` card payment runs through Stripe's payment iframe and is verified manually, not in CI. Turnstile uses Cloudflare's always-pass test site key (the config sets it); CAPTCHA stays off in Supabase, so tokens are accepted-and-ignored. |
