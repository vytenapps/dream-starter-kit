...
| `paywall.spec.ts`       | Anonymous-first identity + the premium-content paywall: favoriting as a logged-out visitor mints a real anonymous account (`public.content_favorites`); a premium post is gated (blurred body + on-page dock); a free user gaining an active subscription unlocks the body (the `usePremium` gating flip); and — when a test-mode `STRIPE_SECRET_KEY` is set — the billing `express-intent` route (the embedded paywall's backend) creates a real Stripe subscription intent. The full embedded Apple/Google-Pay + `4242` card payment runs through Stripe's payment iframe and is verified manually, not in CI. The suite runs with Supabase CAPTCHA off, so anon sign-in/favorites need no token; to exercise the Turnstile widget locally set `NEXT_PUBLIC_TURNSTILE_SITE_KEY` to Cloudflare's always-pass test key in `.env` (see `docs/TURNSTILE.md`) — but note it keeps pages from reaching `networkidle`, which a few specs wait on. |
