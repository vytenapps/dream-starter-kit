...
export async function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith("/admin") || pathname.startsWith("/cms-api")) {
    // Without its env the CMS can't even init — Payload throws "missing
    // secret key" and the founder gets an opaque 500 digest. Answer with the
    // exact missing var names instead (names only — safe for anonymous eyes,
    // and exactly what a founder debugging a fresh deploy needs).
    const missing = missingCmsEnv();
    if (missing.length > 0) {
      return new NextResponse(cmsNotConfiguredMessage(missing), {
        status: 503,
